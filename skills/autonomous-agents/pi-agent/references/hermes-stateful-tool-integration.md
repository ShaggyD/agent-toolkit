# Hermes-native `pi_agent` tool integration notes
...
1. Create/update tool module: `~/.hermes/hermes-agent/tools/pi_agent_tool.py`.
2. Register schema as `pi_agent` with the action API above.
3. Persist session metadata to: `~/.hermes/state/pi_agent_sessions.json`.
4. Expose in toolsets by adding `pi_agent` to:
   - `_HERMES_CORE_TOOLS`
   - `terminal` toolset
5. Verify registration with `registry.get_entry("pi_agent")`.
...
- Add focused tests under `tests/tools/test_pi_agent_tool.py`.
- Also update `tests/tools/test_registry.py` expected builtin tool list to include `tools.pi_agent_tool`.
- Run:
  - `python -m pytest tests/tools/test_pi_agent_tool.py -q -o 'addopts='`
  - `python -m pytest tests/tools/test_pi_agent_tool.py tests/tools/test_registry.py -q -o 'addopts='`
